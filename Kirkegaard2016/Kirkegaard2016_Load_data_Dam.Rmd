...
```{r Load_data}
S1 <- read.csv("data/LIB-MA-28-A-1_TAAGGCGA_L003_R1_001 (paired) trimmed (paired) mapping.csv", header = T)               
S2 <- read.csv("data/LIB-MA-29-A-1_CGTACTAG_L003_R1_001 (paired) trimmed (paired) mapping.csv", header = T)
#S3 <- read.csv("data/S32.csv", header = T)
...
a16S<-read.delim("data/16S.csv", header = T, sep = ";")
a23S<-read.delim("data/23S.csv", header = T, sep = ";")

assembly <- readDNAStringSet("data/assembly.fa", format = "fasta")

network <- read.delim("data/condensed.cytoscape.connections.tab", header = T)
