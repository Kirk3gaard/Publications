...
status<-try(load("data.RData"),silent = T)
 if(class(status)=="try-error") {
   ...
   status<-try(load(url("https://s3-eu-west-1.amazonaws.com/pfigshare-u-files/7025558/data.RData")))
    if(class(status)=="try-error") {
      rm(list = ls())
...
other attached packages:
 [1] plotly_4.5.6               plyr_1.8.4                
 [3] cowplot_0.7.0              ampvis_1.27.0             
 [5] scales_0.4.1               gridExtra_2.2.1           
 [7] magrittr_1.5               dplyr_0.5.0               
 [9] stringr_1.1.0              ggdendro_0.1-20           
[11] DESeq2_1.14.1              SummarizedExperiment_1.4.0
[13] Biobase_2.34.0             GenomicRanges_1.26.2      
[15] GenomeInfoDb_1.10.2        data.table_1.10.0         
[17] Biostrings_2.42.1          XVector_0.14.0            
[19] IRanges_2.8.1              S4Vectors_0.12.1          
[21] BiocGenerics_0.20.0        RColorBrewer_1.1-2        
[23] igraph_1.0.1               vegan_2.4-2               
[25] lattice_0.20-34            permute_0.9-4             
[27] phyloseq_1.19.1            reshape2_1.4.2            
[29] ggrepel_0.6.5              ggplot2_2.2.1             
[31] knitr_1.15.1              

loaded via a namespace (and not attached):
 [1] httr_1.2.1           tidyr_0.6.1          viridisLite_0.1.3   
 [4] jsonlite_1.2         splines_3.3.2        foreach_1.4.3       
 [7] Formula_1.2-1        assertthat_0.1       latticeExtra_0.6-28 
[10] yaml_2.1.14          RSQLite_1.1-2        backports_1.0.5     
[13] digest_0.6.11        checkmate_1.8.2      colorspace_1.3-2    
[16] htmltools_0.3.5      Matrix_1.2-7.1       XML_3.98-1.5        
[19] genefilter_1.56.0    zlibbioc_1.20.0      purrr_0.2.2         
[22] xtable_1.8-2         BiocParallel_1.8.1   htmlTable_1.8       
[25] tibble_1.2           annotate_1.52.1      mgcv_1.8-15         
[28] nnet_7.3-12          lazyeval_0.2.0       survival_2.40-1     
[31] memoise_1.0.0        evaluate_0.10        nlme_3.1-128        
[34] MASS_7.3-45          foreign_0.8-67       tools_3.3.2         
[37] munsell_0.4.3        locfit_1.5-9.1       cluster_2.0.5       
[40] AnnotationDbi_1.36.1 ade4_1.7-5           rhdf5_2.18.0        
[43] RCurl_1.95-4.8       iterators_1.0.8      biomformat_1.2.0    
[46] htmlwidgets_0.8      bitops_1.0-6         base64enc_0.1-3     
[49] rmarkdown_1.3        gtable_0.2.0         codetools_0.2-15    
[52] multtest_2.30.0      DBI_0.5-1            R6_2.2.0            
[55] Hmisc_4.0-2          rprojroot_1.2        ape_4.0             
[58] stringi_1.1.2        Rcpp_0.12.9          geneplotter_1.52.0  
...
```{r,fig.align="center",fig.height=13,fig.width=7}
n_taxa<-20
...
plot_grid(p5_a, p5_b, align='vh',ncol = 1, labels=c('A', 'B'))
fname<-paste0("Figure4.pdf")

#ggsave(filename = fname,width = 7,height = 13, useDingbats = FALSE)
```

Get stats about the number of samples per sample type
...
## Figure 5
Digester abundance for the OTUs versus the ratio between the abundance in the digester compared to the abundance in the influent streams (primary and surplus sludge)
...
#pdf(file = "Figure5_OTUlabels.pdf",width = 10, height = 5.5)#10*10
...
# Save the figure
#ggsave(filename = "Figure6.pdf",width = 10,height = 7, useDingbats = FALSE)
```
...
# Save figure
#ggsave(filename = "FigureS2.pdf",width = 18,height = 9, useDingbats = FALSE)
```
...
# Save figure
#ggsave(filename = "FigureS3.pdf",plot = p,width = 20,height = 45, useDingbats = FALSE)
```
...
#ggsave(filename = "FigureS4.pdf",width = 5,height = 5, useDingbats = FALSE)
```
...
```{r, fig.align="center",fig.height=7,fig.width=17}
...
#ggsave(filename = "FigureS6.pdf",width = 17,height = 7, useDingbats = FALSE)
```

Get stats about the number of samples per plant

```{r}
mytable<-sample_data(PS)
summary(factor(mytable$Sample_location))

length(factor(mytable$Sample_location))
```
...
#ggsave(filename = "FigureS8.pdf",width = 10,height = 7, useDingbats = FALSE)
```
